...
1. usuario_id: Identificador único para cada usuario.
2. usuario_nombre: Nombre del usuario.
3. usuario_edad: Edad del usuario.
...
The goal is to design personalized campaigns and optimize future marketing actions. To achieve this, customers must be segmented based on variables such as age, purchase history, and the categories of products they have purchased.

As a member of the analytics team, your initial task is to review and transform a sample of the existing data so that it is ready for analysis. In this phase, you will:

1. Clean customer profiles
2. Standardize names and ages
3. Calculate total spending per customer
4. Validate data consistency and correct errors
5. Prepare the data to generate business indicators (KPIs)

The data provided by Store 1 is formatted as a Python list and includes the following columns:

1. usuario_id: Unique identifier for each user
2. usuario_nombre: User name
3. usuario_edad: User age
4. categorias_fav_low: Favorite categories of items purchased by the user, such as ELECTRONICS, SPORTS, BOOKS, etc.
5. gasto_por_categoria: A list of integers indicating the total amount spent in each favorite category


categorias_fav_low: Favorite categories of items purchased by the user, such as ELECTRONICS, SPORTS, BOOKS, etc.

gasto_por_categoria: A list of integers indicating the total amount spent in each favorite category
